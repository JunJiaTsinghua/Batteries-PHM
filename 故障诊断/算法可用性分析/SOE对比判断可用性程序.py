...
def main():
    ...
    if max_cycle_num<30: # 右边那个循环
        ...
        #SOC_SOE方法的可用性
        if "SOC_SOE" not in 数据库 # TODO 这个不属于中间特征，属于是属性信息，导入时候填的那个，一般都没有
            res[0]["button_SOC_SOE_available"]= 0
        else:
            res[0]["button_SOC_SOE_available"] = 1
        #安时积分法的可用性
        ...
        # 判断可用性
        if all_features_compute_flag["容量"]:
            res[0]["button_Ah_available"] = 1
        else:
