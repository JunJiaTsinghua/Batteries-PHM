function  [Calculation_results,flags]  = function_min_voltage(data,flags,work_folder_file_folder,Calculation_results)
%function_min_voltage 所选时间范围内，温度最高值
global data_struct
data_struct=data;
    ...
    for o=flags.analyze_object
        ...
        if isfield(flags.(char(o)),'Vol')
            if isfield(flags.(char(o)).Vol,'Diff')
                M_data=flags.(char(o)).Vol.Diff.min;
                %没有算过，就要现场算。
            else
        ...
        if isa(M_data,'char')
            ...
            min_min_vol=min(M_data,[],1);%每个模组/簇/舱最小值里面的最小值，报告只输出这一个
            report_content=mat2str(round(min_min_vol,-2));
        end
       ...
       switch plot_way
           case '分开画'
               for k=1:size(M_data,2)
                   h_fig= figure('name',[char(o),int2str(k),'最低电压曲线'],'NumberTitle','off','Visible','off');
                   plot(M_data(:,k))
                   xlabel('数据采集点');
                   ylabel('最低电压(V)');
                   title([char(o),int2str(k),'最低电压曲线'])
                   set(h_fig,'Visible','on')
                   saveas(h_fig,[work_folder_file_folder,'\',[char(o),int2str(k),'最低电压曲线'],'.fig'])
                   close(h_fig)
                   this_function_figs=[this_function_figs,work_folder_file_folder,'\',char(o),int2str(k),'最低电压曲线','.fig' ,  10  ...
                ];
               end
               
           case '一起画'
               h_fig= figure('name',[char(o),'最低电压曲线'],'NumberTitle','off','Visible','off');
               plot(M_data)
               legend_cell={};
               for k=1:size(M_data,2)
               ...
               set(h_fig,'Visible','on')
               saveas(h_fig,[work_folder_file_folder,'\',[char(o),'最低电压曲线'],'.fig'])
               close(h_fig)
               this_function_figs=[this_function_figs,work_folder_file_folder,'\',char(o),'最低电压曲线','.fig' ,  10  ...
                   ];
       end
