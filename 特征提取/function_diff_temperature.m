...
%function_diff_temperature 所选时间范围内，温度差值
global data_struct
data_struct=data;
...
try
    for o=flags.analyze_object
        ...
        if isfield(flags.(char(o)),'Tem')
            if isfield(flags.(char(o)).Tem,'Diff')
                ...
                flags.(char(o)).Tem.Diff=result_data;%有想过不再分Diff和Incon这一层，但是，返回的值没法直接赋给Tem或Tem，不方便
        ...
        if isa(M_data,'char')
            ...
            max_diff_Tem=max(M_data,[],1);%每个模组/簇/舱温度差里面的最大值，报告只输出这一个
            report_content=mat2str(roundn(max_diff_Tem,-2));
        end
         this_function_records=[this_function_records, flags.data_part,char(o),'中各',sub_layer,'间最大温差:',report_content ,'     '];
        %图片绘制-分开还是hold on
       ...
       switch plot_way
           case '分开画'
               for k=1:size(M_data,2)
                   h_fig= figure('name',[char(o),int2str(k),'中各',sub_layer,'间最大温差曲线'],'NumberTitle','off','Visible','off');
                   plot(M_data(:,k))
                   xlabel('数据采集点');
                   ylabel('最大温差(℃)');
                   title([char(o),int2str(k),'中各',sub_layer,'间最大温差曲线'])
                   set(h_fig,'Visible','on')
                   saveas(h_fig,[work_folder_file_folder,'\',[char(o),int2str(k),'中各',sub_layer,'间最大温差曲线'],'.fig'])
                   close(h_fig)
                   this_function_figs=[this_function_figs,work_folder_file_folder,'\',char(o),int2str(k),'中各',sub_layer,'间最大温差曲线','.fig' ,  10  ...
                ];
               end
               
           case '一起画'
               h_fig= figure('name',[char(o),'中各',sub_layer,'间最大温差曲线'],'NumberTitle','off','Visible','off');
               plot(M_data)
               legend_cell={};
               for k=1:size(M_data,2)
               ...
               xlabel('数据采集点');
               ylabel('最大温差(℃)');
               title([char(o),'中各',sub_layer,'间最大温差曲线'])
               if length(legend_cell)>1
                 legend(legend_cell)
               end
               set(h_fig,'Visible','on')
               saveas(h_fig,[work_folder_file_folder,'\',[char(o),'中各',sub_layer,'间最大温差曲线'],'.fig'])
               close(h_fig)
               this_function_figs=[this_function_figs,work_folder_file_folder,'\',char(o),'中各',sub_layer,'间最大温差曲线','.fig' ,  10  ...
                   ];
       end
    ...
    %输出报告
    % 功能点
    Calculation_results(size(Calculation_results,2)+1).name = '最大温差';
    %结果摘要
