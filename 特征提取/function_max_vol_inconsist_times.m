function  [Calculation_results,flags]  = function_max_vol_inconsist_times(data,flags,work_folder_file_folder,Calculation_results)
%function_max_vol_inconsist_times 所选时间范围内，电压不一致性最大出现在某个对象的次数
global data_struct
data_struct=data;
...
%                         end
        ...
        if isa(In_data,'char') || size(In_data,2)==1
            report_content=['  ',father_layer,'电压',inconsist_method,'仅有一组计算结果,无法计算差异性特征量'];
        else
            ...
            report_content='';
            report_content=[report_content,'   ',father_layer,'电压',inconsist_method,'中最大次数最多的三个',char(o),':',mat2str(MAX_probes)];
        ...
        if output_figs && ~isa(In_data,'char') && size(In_data,2)>1
            h_fig= figure('name',[father_layer,'电压',inconsist_method,'中各',char(o),'出现最高不一致性的次数'],'NumberTitle','off','Visible','off');
            
            bar(max_diff_vol_times(:,1),max_diff_vol_times(:,2))
            xlabel([char(o),'编号']);
            ylabel('最高不一致性次数');
            title([father_layer,'电压',inconsist_method,'中各',char(o),'出现最高不一致性的次数'])
            set(h_fig,'Visible','on')
            saveas(h_fig,[work_folder_file_folder,'\',[father_layer,'电压',inconsist_method,'中各',char(o),'出现最高不一致性的次数'],'.fig'])
            close(h_fig)
            this_function_figs=[this_function_figs,work_folder_file_folder,'\',[father_layer,'电压',inconsist_method,'中各',char(o),'出现最高不一致性的次数'] ,  10  ...
                ];
        end
    ...
    %输出报告
    % 功能点
    Calculation_results(size(Calculation_results,2)+1).name = '最大电压不一致性次数统计';
    %结果摘要
