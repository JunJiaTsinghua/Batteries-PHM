function [Calculation_results,flags]= function_charge_max_temperature(data,flags,work_folder_file_folder,Calculation_results)
%function_charge_max_temperature 单次充入电量
global data_struct
data_struct=data;
...
try
    for o=flags.analyze_object
        ...
        %报告内容计算
             charge_max_tem=zeros(length(charge_index),size(Tem_data,2));
            ...
            for i =1:length(charge_index)
                ...
                charge_max_tem(i,:)=max_Tem;
                [max_Tem,probe]=max(max_Tem);
                if size(Tem_data,2)==1
                    this_requipment_num='';
                else
                    this_requipment_num=int2str(probe);
                end
                this_function_records=[this_function_records, '循环',int2str(i),'最大温度是',char(o),this_requipment_num,':',num2str( roundn(max_Tem,-2) ),'℃   '];
            end
  
        %图片绘制-分开还是hold on
        if output_figs
            plot_way=flags.plot_split;
            if size(charge_max_tem,2)==1
                plot_way='一起画';
            end
       switch plot_way
           case '分开画'
               for k=1:size(charge_max_tem,2)
                   h_fig= figure('name',[char(o),int2str(k),'充电最大温度变化曲线'],'NumberTitle','off','Visible','off');
                   switch charge_progress_X_axis
                       case '次数'
                           plot(charge_max_tem(:,k))
                       case '里程'
                           plot(mileage_axis,charge_max_tem(:,k))
                   end
                   xlabel(charge_progress_X_axis);
                   ylabel('最大温度(℃)');
                   title([char(o),int2str(k),'充电最大温度变化曲线'])
                   set(h_fig,'Visible','on')
                   saveas(h_fig,[work_folder_file_folder,'\',[char(o),int2str(k),'充电最大温度变化曲线'],'.fig'])
                   close(h_fig)
                   this_function_figs=[this_function_figs,work_folder_file_folder,'\',char(o),int2str(k),'充电最大温度变化曲线','.fig' ,  10  ...
                ];
               end
               
           case '一起画'
               h_fig= figure('name',[char(o),'充电最大温度变化曲线'],'NumberTitle','off','Visible','off');
               switch charge_progress_X_axis
                   case '次数'
                       plot(charge_max_tem)
                   case '里程'
                       plot(mileage_axis,charge_max_tem)
               end
               legend_cell={};
               for k=1:size(charge_max_tem,2)
               ...
               xlabel(charge_progress_X_axis);
               ylabel('最大温度(℃)');
               title([char(o),'充电最大温度变化曲线'])
               if length(legend_cell)>1
                 legend(legend_cell)
               end
               set(h_fig,'Visible','on')
               saveas(h_fig,[work_folder_file_folder,'\',[char(o),'充电最大温度变化曲线'],'.fig'])
               close(h_fig)
               this_function_figs=[this_function_figs,work_folder_file_folder,'\',char(o),'充电最大温度变化曲线','.fig' ,  10  ...
                   ];
       end
    ...
    %输出报告
    % 功能点
    Calculation_results(size(Calculation_results,2)+1).name = '充电过程最大温度';
    %结果摘要
